...
/**
 * Парсит JSON из PDF-to-JSON с полем "lines": string[] или "segments": string[].
...
const OPTION_REGEX = /^[A-EАБВГДЕ]\)\s*/i;
...
const NEW_PASSAGE_TEXT_REGEX = /^Text\s+\d+\s*:/i;

function isNewPassageStart(trimmed: string): boolean {
  return (
    NEW_PASSAGE_BLOCK_REGEX.test(trimmed) ||
    NEW_PASSAGE_TEKST_REGEX.test(trimmed) ||
    NEW_PASSAGE_TEXT_REGEX.test(trimmed)
  );
}
...
const KEY_SECTION_START =
  /КЛЮЧИ|КЛЮЧ\s+ОТВЕТОВ|^КЛЮЧ\s*$|ОТВЕТЫ|ANSWERS|ЖАУАПТАР|ЖАУАП\s+КІЛТТЕРІ|КІЛТТЕРІ\s*\(|ТҮСІНДІРМЕ|KEY\s*\(/i;
...
const KEY_LINE_WITH_DASH_REGEX = /^\s*(\d+)\.\s*([A-EАБВГДЕ])\.?\s*[—\-]\s*(.+)$/;
/** English key: "2. B. Detail: ..." or "1. C ($21). ..." */
const KEY_LINE_WITH_TAIL_REGEX = /^\s*(\d+)\.\s*([A-EАБВГДЕ])\.?\s+(.+)$/;
...
function getAnswerKeyWithExplanations(lines: string[]): Map<number, { letter: string; explanation?: string }> {
  ...
  for (let i = keyStart + 1; i < lines.length; i++) {
    ...
    const withDash = trimmed.match(KEY_LINE_WITH_DASH_REGEX);
    const withTail = trimmed.match(KEY_LINE_WITH_TAIL_REGEX);

    if (withDash) {
      flush();
      currentNum = parseInt(withDash[1], 10);
      currentLetter = withDash[2];
      currentExpl = [withDash[3]];
      continue;
    }
    if (withTail && !withExpl) {
      flush();
      currentNum = parseInt(withTail[1], 10);
      currentLetter = withTail[2];
      currentExpl = [withTail[3]];
      continue;
    }
    if (onlyKey) {
      flush();
...
export function parseLinesFormat(data: { lines?: string[]; segments?: string[] }): {
  questions: ParsedQuestion[];
  answerKey: Map<number, string>;
  passage: string;
  blocks: BankBlock[];
} {
  const lines = bankJsonToLines(data);

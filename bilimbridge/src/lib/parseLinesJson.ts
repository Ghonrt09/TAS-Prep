...
/** Английский НИШ: «Text 1:», «Text 2:». */
const NEW_PASSAGE_ENGLISH_REGEX = /^Text\s+\d+\s*:/i;
...
function isNewPassageStart(trimmed: string): boolean {
  return (
    ...
    NEW_PASSAGE_ENGLISH_REGEX.test(trimmed)
  );
}
...
/** Начало блока ключей в файле (рус/каз) — только в начале строки, не подстрока в тексте («исключительно»). */
const KEY_SECTION_START =
  /^(КЛЮЧИ\b|КЛЮЧ\s*И\s+ПОЯСНЕН|ОТВЕТЫ\b|ANSWERS\b|ЖАУАПТАР\b|ЖАУАП\s+КІЛТТЕРІ|КІЛТТЕРІ\s*\(|ТҮСІНДІРМЕ\b)/i;
...
export function parseLinesFormat(data: { lines?: string[] }): {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (current && trimmed) {
      ...
      } else if (isNewPassageStart(trimmed)) {
        const qText = current.questionLines.join(" ").replace(/^\d+\.\s*/, "").trim();
        if (!EXAM_HEADER_REGEX.test(qText)) {
          const correctLetter =
            answerKey.get(current.num) ?? keyWithExplanations.get(current.num)?.letter;
          const correctIdx = correctLetter !== undefined ? letterToIndex[correctLetter] : 0;
          const correctAnswer = current.options[correctIdx] ?? current.options[0] ?? "";
          pushQuestion({
            id: current.num,
